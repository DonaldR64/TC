const DnD = (() => {
    ...
    const Markers = {
        ...
        "Guiding Bolt": "Aid::1431844",
        "Shillelagh": "Effect_Nature_Leaf::1431993",
        "Protection from Fire": "Slimed-Red-Transparent::2006582",
        "Protection from Acid": "Slimed-Mustard-Transparent::2006560",
        "Protection from Lightning": "Slimed-Gray-Transparent::2006556",
    ...
    class Model {
        constructor(token) {
            ...
            if (this.sheetType === "NPC") {
                ...
                if (aa.pb_type === "custom") {
                    this.pb = parseInt(aa.pb_custom);
                }
            }
            ...
            this.pronoun = aa.pronoun || "It ";
            this.adjective = " its ";
            if (this.pronoun === " his ") {
                this.adjective = " his ";
                this.pronoun = "He ";
            } else if (this.pronoun === " her ") {
                this.adjective = " her ";
                this.pronoun = "She ";
            }
...
log(model.name + ": " + id)
    ...
    const LineTargets = (caster,target,line) => {
        let temp = [];
        let width = parseInt(line.replace("Line","")) || 1;
        let squares = Line(caster.Point(),target.Point());
        _.each(ModelArray,model => {
            if (model.id === target.id || model.id === caster.id || model.layer === "map") {return}
            if (Venn(squares,model.Squares()) === true) {
                temp.push(model.id);
            }
//width > 1 square
...
log(defender.vulnerabilities)
        ...
        if (savingThrow && savingThrow !== "No" && immune === false && savingThrow !== "auto") {
            ...
            } else if (result.save === "NA") {
                if (saveEffect === "No Damage") {
                    saveTip = "No Damage";
                }
                if (saveEffect === "Half Damage") {
                    saveTip = "Half Damage (" + Math.round(total/2) + ")";
                }
                if (saveEffect === 'Special') {
                    saveTip = damageInfo.saveText;
                }
                save = "NA";
            }
        }
...
log(state.DnD.spells)
    ...
    const Save = (model,dc,stat,adv = 0) => {
        ...
        if (dc === false) {
            ...
            if (fail === true) {
                saved = false,
                saveTip = "[B]Automatically Failed Save due to " + failReason + "[/b]";
            } else {
                outputCard.body.push("[B]" + model.name + " must make a " + Capit(stat) + " Save - DC " + dc + "[/b]");
                let advNoun = (finalAdv >= 0) ? "Advantage":"Disadvantage"; 
                let advTip = '[' + advNoun + '](#" class="showtip" title="' + advReasons.toString() + ')';
                if (finalAdv !== 0) {
                    outputCard.body.push(Capit(model.pronoun) + " has " + advTip);
                }
                saved = "NA";
            }
        }
...
log(weapon)
        ...
        if (weaponName === "Spiritual Weapon") {
            ...
            let spell = state.DnD.spellList.find((e) => e.spellID == spellID);
            if(!spell) {
                sendChat("","Error with Routine")
                return
            };
            attacker = ModelArray[spell.casterID];
            statBonus = attacker.statBonus["wisdom"];
            weapon.magic = "magic";
        ...
        if (weapon.info.includes("Silver") && weapon.magic.includes("magic") === false) {
            weapon.magic = "silver";
        }
        ...
        if (inReach === true && weapon.type.includes("Melee")) {
            outputCard.body.push(attacker.name + " strikes at " + defender.name + " with" + attacker.adjective + weaponName);
            weapon.type = "Melee";
        }
        if (inReach === false && weapon.properties.includes("Thrown")) {
            outputCard.body.push(attacker.name + " throws" + attacker.adjective + weaponName + " at " + defender.name);
            weapon.sound = "Shuriken";
            weapon.type = "Ranged";
        }
        if (inReach === false && weapon.properties.includes("Thrown") === false) {
            outputCard.body.push(attacker.name + ' fires' + attacker.adjective + weaponName + " at " + defender.name);
            weapon.type = "Ranged";
        }
...
log(weapon)
    ...
    const Advantage = (attacker,defender,damageInfo) => {
        ...
        if (damageInfo.type.includes("Melee") === false && attacker.special.includes("Close Quarters Fighter") === false) {
        ...
        if (defMarkers.includes("Guiding Bolt")) {
            advantage = true;
...
//resource
        ...
        if (abilityName === "Healer") {
            let defID = Tag[3];
            let defender = ModelArray[defID];
            SetupCard(attacker.name,"Healer",attacker.displayScheme);
            let roll = randomInteger(6);
            let healed = roll + 4 + defender.level
            let tip = "Roll: [" + roll + "] + 4 + " + defender.level;
            tip += "<br>[d6] + 4 + Target's HD";
            tip = '[' + healed + '](#" class="showtip" title="' + tip + ')';

            outputCard.body.push(defender.name + " is Healed for " + tip + " Wounds");
    ...
    const Spell = (msg) => {
        ...
        if (!SpellInfo[spellName]) {
            outputCard.body.push("Not in Database Yet");
            PrintCard();
            return;
        }
        ...
        if (spellName === "Hold Person") {
            let target = ModelArray[targetIDs[0]];
            if (target.type.includes("human") === false) {
...
log(spell)
        ...
        if (spell.name === "Protection from Energy") {
            let marker = spell.marker;
...
log("in party: " + m.inParty)
    ...
    const SpellDamage = (rollResults,spell,model) => {
        ...
        if (!damageResults.save) {
            ...
            outputCard.body.push(model.name + ' [' + damageResults.save + '](#" class="showtip" title="' + damageResults.saveTip + ')' + " and" +" takes [#ff0000]" + damageResults.total + "[/#] Damage" + damageResults.irv);
        }
...
//
        ...
        if (ritualCasters.includes(model.class)) {
            ...
            for (let i=1;i<10;i++) {
                ...
                if (buttons.length > 0) {
                    rituals.push(InlineButtons(buttons));
                    rituals.push("<br>");
                    rituals.push("[hr]");
                }
            }
log(rituals)
    ...
    const MiscSpell = (spell) => {
        ...
        let list = ["Conjure Animals"]; //dont need targets
        let someoneFailed = (list.includes(spell.name)) ? true:false;
        ...
        _.each(spell.targetIDs,targetID => {
            ...
            if (spell.savingThrow && !spell.beneficial) {
                ...
                saved = saveResult.save;
                let noun = (saved === true) ? "Saves":"Fails";
                let text = (saved === true) ? spell.saveText:spell.failText;
                if (saved !== "NA") {
        ...
        if (spell.name === "Protection from Energy") {
            let type = spell.energyType;
            let marker = "status_" + Markers["Protection from " + type];
            spell.marker = marker;
            let target = ModelArray[spell.targetIDs[0]];
            target.token.set(marker,true);
        }
    ...
    const SpellTarget = (spell) => {
        ...
        if (spell.name === "Spiritual Weapon") {
            action = "!Attack;@{selected|token_id};@{target|token_id};Spiritual Weapon;" + spell.base + ",force;;Melee;;;;Smite";
        ...
        if (newToken) {
            ...
            target.inParty = spell.inParty;
            state.DnD.areaSpell = spell;
            spell.tokenID = newToken.get("id");
    ...
    const AreaSpell = (msg) => {
        ...
        } else if (spell.area.includes("Line")) {
            targets = LineTargets(caster,spellTarget,spell.area);
        }
        ...
        if (spell.areaEffect && spell.areaEffect.includes("Effect")) {
            if (spell.failTextAll) {
                outputCard.body.push(spell.failTextAll);
                outputCard.body.push("[hr]");
            }
...
log(state.DnD.spellList)
    ...
    const SetPronoun = (msg) => {
        if (!msg.selected) {return}
        let id = msg.selected[0]._id;
        let adj = msg.content.split(";")[1];
        let pronoun = (adj === "his") ? "He ":"She ";
        adj = " " + adj + " ";
        let model = ModelArray[id];
        if (model) {
            AttributeSet(model.charID,"pronoun",pronoun);
            AttributeSet(model.charID,"adjective",adj);
            sendChat("","Set to " + pronoun + "/" + adj);    
        }

    }

    const SpellMove = (model) => {
        let spellIDs = [];
        if (state.DnD.conSpell[model.id]) {
            spellIDs.push(state.DnD.conSpell[model.id]);
        }
        ...
        for (let i=0;i<spellIDs.length;i++) {
            let spellID = spellIDs[i];
            let spell = state.DnD.spellList.find((e) => e.spellID === spellID);
            if (spell.moveWithCaster && spell.moveWithCaster === true) {
                let spellToken = findObjs({
                    _pageid: Campaign().get("playerpageid"),
                    _type: "graphic",
                    _subtype: "token",
                    id: spell.tokenID,
...
log("Is Spell: " + model.isSpell)
    ...
    const ApplyMarker = (msg) => {
        if (!msg.selected) {
            sendChat("API","/w GM Select PC")
            return
        } 
        let id = msg.selected[0]._id;
        if (state.DnD.saveMarkers[id]) {
            let marker = "status_" + Markers[state.DnD.saveMarkers[id]];
            ModelArray[id].token.set(marker,true);
            state.DnD.saveMarkers[id] = "";
        } else {
            sendChat("API","/w GM No Marker")
        }
    }
